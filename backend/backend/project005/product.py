...
def newproduct(request):
    try:
        ...
        query(db,"INSERT INTO `product`(`coverurl`,`title`,`content`,`tag`,`createtime`,`updatetime`)VALUES(%s,%s,%s,%s,%s,%s)",["project005/"+covername,title,content,tag,time(),time()])
...
def getproductlist(request,sorttype):
    try:
        if sorttype=="all":
            ...
            row=query(db,"SELECT*FROM `product` WHERE `tag`=%s",[sorttype])
